...
\tightlist
\item
  A FITS-verification bug. ~This does not appear to be fatal and is not
  a blocker, but should be fixed.
\item
...
These are not obstacles to commissioning or Data Preview work; the
system is usable as-is. However, these issues should be addressed with
priority.\\[2\baselineskip]A few notable points also resulted in tickets
but were rated ``Pass'':
...
\item
  ...
  images;
\item
  The lack of a grid view for the extensions in raw images.
\end{itemize}
...
changing of stretch functions. Investigate the speed of mask display and
of initial loading of HiPS images.\\[2\baselineskip]Work on PyVO access
to the image services and on possible ``convenience'' wrapper functions
(as we already have for TAP) between now and ComCam operation.
...
verified.\\[2\baselineskip]Place in MONITORING state.
...
needs improvement: leave in MONITORING state.
...
provided.\\[2\baselineskip]Assessing as ``Initial Pass'' because of the
missing median, specifically. This must be
addressed.\\[2\baselineskip]Comments:
...
\hdashrule[0.5ex]{\textwidth}{1pt}{3mm}
  Actual Result \\
{\footnotesize
This workaround was successful at producing a query for only coadds.
